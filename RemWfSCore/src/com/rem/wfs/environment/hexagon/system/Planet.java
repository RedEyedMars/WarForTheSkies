...
public class Planet extends GraphicEntity implements Storable, Identifiable {
	...
	public void reposition(float x, float y){
		double radius = Math.sqrt(Math.pow(x-(system.getX()+system.getWidth()/3f),2)
				+Math.pow(y-(system.getY()+system.getHeight()/3f),2));
		x=(float) (system.getX()+system.getWidth()/3f+Math.cos(angleToCenter)*radius);
		y=(float) (system.getY()+system.getHeight()/3f+Math.sin(angleToCenter)*radius);

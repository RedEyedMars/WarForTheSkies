...
public class GLImage {
    ...
    public GLImage(String imgName){
        ...
        if (makeGLImage(img,false,false)) {
    ...
    {
		BufferedImage img = loadJavaImage(inputStream);
        if (makeGLImage(img,true,false)) {
			GLApp.msg("GLImage(InputStream): loaded " + inputStream + ", width=" + w + " height=" + h);
		}
    }
    ...
    public GLImage(InputStream imgName, boolean flipYaxis, boolean convertPow2)
    {
		BufferedImage img = loadJavaImage(imgName);
        if (makeGLImage(img,flipYaxis,convertPow2)) {
			GLApp.msg("GLImage(String,bool,bool): loaded " + imgName + ", width=" + w + " height=" + h);
    ...
    public GLImage(BufferedImage img) {
        if (makeGLImage(img,false,false)) {
			GLApp.msg("GLImage(BufferedImage): loaded image from bufferedImage");
		}
		else {
			GLApp.err("GLImage(BufferedImage): could not create Image from bufferedImage");
		}
    }
	 ...
	 */
    public static BufferedImage loadJavaImage(String filename) {
    	BufferedImage tmpi = null;
    	try {
    		tmpi = ImageIO.read(new File(filename));
    	}
    	catch (Exception e) {
    ...
    public static BufferedImage loadJavaImage(InputStream inputStream) {
    	BufferedImage tmpi = null;
    	try {
    		tmpi = ImageIO.read(inputStream);
    		inputStream.close();
    	}
    	catch (Exception e) {
    		GLApp.err("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE " + e);
